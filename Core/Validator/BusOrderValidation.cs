    ...
    public class BusOrderValidation : AbstractValidator<SaveBusOrderResource>
    {
        ...
        private bool IsDateAcceptable(SaveBusOrderResource resource)
        {
            if (!string.IsNullOrEmpty((resource.OrderEntryDate).ToString()))
            {
                ...
                var timeLockStart1 = AppConfig.LockedBusOrderStart1;
                var timeLockEnd1 = AppConfig.LockedBusOrderEnd1;
                var timeLockStart2 = AppConfig.LockedBusOrderStart2;
                var timeLockEnd2 = AppConfig.LockedBusOrderEnd2;

                if (DateTime.Now.TimeOfDay > TimeSpan.Parse(timeLockStart1) && DateTime.Now.TimeOfDay < TimeSpan.Parse(timeLockEnd1))
                {
                    return false;
                }

                if (DateTime.Now.TimeOfDay > TimeSpan.Parse(timeLockStart2) && DateTime.Now.TimeOfDay < TimeSpan.Parse(timeLockEnd2))
                {
                    return false;
                }
